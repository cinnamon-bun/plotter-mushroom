...
const md5 = __webpack_require__(31);
//================================================================================
exports.range = (x) => {
    let result = [];
    for (let n = 0; n < x; n++) {
...
exports.detChoose = (s, arr) => 
// deterministically choose a random-ish item from an array based on the value of s
arr[Math.floor(exports.detRandom(s) * arr.length)];
exports.remap = (x, oldMin, oldMax, newMin, newMax) => {
    let ii = (x - oldMin) / (oldMax - oldMin);
    return ii * (newMax - newMin) + newMin;
};
exports.clamp = (x, minn, maxx) => Math.max(minn, Math.min(maxx, x));
exports.remapAndClamp = (x, oldMin, oldMax, newMin, newMax) => exports.clamp(exports.remap(x, oldMin, oldMax, newMin, newMax), newMin, newMax);
exports.mySin = (x, period, phase, minn, maxx) => exports.remap(Math.sin((x / period - phase) * Math.PI * 2), -1, 1, minn, maxx);
exports.linterp = (v0, v1, blend) => (1 - blend) * v0 + blend * v1;
exports.sinterp = (v0, v1, blend) => exports.remap(Math.cos(blend * Math.PI), 1, -1, v0, v1);
exports.sawtooth = (x) => exports.posMod(x, 1);
exports.sign = (x) => x >= 0 ? 1 : -1;
exports.findZero = (v0, v1) => {
    ...
    if (exports.sign(v0) === exports.sign(v1)) {
        throw "findZero sign problem";
    }
    return (-v0) / (v1 - v0);
};
exports.myPow = (x, p) => (x > 0)
    ? Math.pow(x, p)
    : -Math.pow(-x, p);
exports.slide = (x, y) => (p) => new Point(p.x - x, p.y - y);
exports.scaleUniform = (s, origin) => (p) => origin
    ? p.subPoint(origin).divFloat(s).addPoint(origin)
    : p.divFloat(s);
exports.scaleXY = (sx, sy, origin) => (p) => origin
...
class Point {
    ...
    clone() { return new Point(this.x, this.y); } // note: does not clone history
    addPoint(p2) { return new Point(this.x + p2.x, this.y + p2.y); }
    subPoint(p2) { return new Point(this.x - p2.x, this.y - p2.y); }
    mulPoint(p2) { return new Point(this.x * p2.x, this.y * p2.y); }
    divPoint(p2) { return new Point(this.x / p2.x, this.y / p2.y); }
    addFloat(n) { return new Point(this.x + n, this.y + n); }
    subFloat(n) { return new Point(this.x - n, this.y - n); }
    mulFloat(n) { return new Point(this.x * n, this.y * n); }
    divFloat(n) { return new Point(this.x / n, this.y / n); }
    transform(tr) { return tr(this); }
    ...
    distTo(p2) { return this.subPoint(p2).len(); }
    mix(p2, pct) { return this.mulFloat(1 - pct).addPoint(p2.mulFloat(pct)); }
    static detRandomInSquare(seed) {
        return new Point(exports.detRandom(seed) * 2 - 1, exports.detRandom(seed + 'zzz') * 2 - 1);
    }
    static detRandomInCircle(seed) {
        let ii = 0;
        while (true) {
...
const plotvis_1 = __webpack_require__(34);
//================================================================================
let makeMushroom = () => {
    /*
    ...
    */
    ...
    let gravity = 0.003;
    let twist = 0.4;
    let brushes = core_1.range(initialNumSeeds).map(ii => {
        let rand = core_1.remap(core_1.detRandom('circlejitter-' + ii + '-' + seed), 0, 1, -0.4, 0.4);
        return new core_1.Point(Math.sin((ii + rand) / initialNumSeeds * 2 * Math.PI), Math.cos((ii + rand) / initialNumSeeds * 2 * Math.PI)).toBrush();
    });
    let rad = 0.0001;
    let paths = {};
    let iter = 0;
    ...
    for (let rad = minRad; rad < maxRad; rad += radStep) {
        ...
        for (let ii = 0; ii < oldPoints.length; ii++) {
            let prevPoint = oldPoints[core_1.posMod(ii - 1, brushes.length)];
            let thisBrush = brushes[ii];
            let nextPoint = oldPoints[core_1.posMod(ii + 1, brushes.length)];
            let jitterMix = core_1.remap(core_1.detRandom('jittermix-' + ii + '-' + rad + '-' + seed), 0, 1, 0.5 - jitter / 2, 0.5 + jitter / 2);
            jitterMix += twist;
            let newPoint = thisBrush.mulFloat(damp)
                .addPoint(prevPoint.mulFloat((1 - damp) * (jitterMix)))
                .addPoint(nextPoint.mulFloat((1 - damp) * (1 - jitterMix)))
                .addPoint(new core_1.Point(wind, -gravity));
        ...
        // split
        let brushesWithSplits = [];
        for (let ii = 0; ii < brushes.length; ii++) {
            brushesWithSplits.push(brushes[ii]);
            if (iiToSplit.indexOf(ii) !== -1) {
                let splitJitterP = core_1.Point.detRandomInCircle('splitjitter-' + ii + '-' + seed).mulFloat(splitJitter);
...
console.log(paths);
//================================================================================
ReactDOM.render(React.createElement("div", { style: { textAlign: 'center' } },
    React.createElement(plotvis_1.PlotVis, { paths: paths, origin: core_1.ORIGIN, radius: 1.2, res: 1000, strokeWidth: 3, stroke: "#234", fill: "transparent" })), document.getElementById('react-slot'));
/*
    paths : Path[];
    origin : Point;
    radius : number;
...
Object.defineProperty(exports, "__esModule", { value: true });
const React = __webpack_require__(2);
const Color = __webpack_require__(35);
const core_1 = __webpack_require__(14);
let pointToScreen = (res, origin, radius, point) => {
    return new core_1.Point(core_1.remap(point.x, origin.x - radius, origin.x + radius, 0, res - 1), core_1.remap(point.y, origin.y + radius, origin.y - radius, 0, res - 1));
};
class PlotVis extends React.Component {
    constructor(props) {
        super(props);
        this.state = {};
    }
    ...
    render() {
        return (React.createElement("svg", { style: this.props.style, width: this.props.res, height: this.props.res },
            React.createElement("rect", { width: this.props.res, height: this.props.res, fill: this.props.fill }),
            this.props.paths.map((path, ii) => {
                return React.createElement("path", { key: 'p' + ii, d: this.pointsToSvgPath(path), stroke: Color(this.props.stroke).lighten(Math.random() * 0.4).toString(), strokeWidth: this.props.strokeWidth, fill: "transparent" });
            })));
    }
}
exports.PlotVis = PlotVis;
